...
def search() -> str:
    ...
    if request.method == "POST":
        ...
        try:
            ...
            logger.warning(
                f"Invalid top_k value received: {top_k_str}. Falling back to 5."
            )
            top_k = 5
...
def chat() -> str:
    ...
    if request.method == "POST":
        ...
        if not question:
            ...
            chat_service = ChatService(qwrap, embedder, chat_history=chat_history)
...
def metrics_dashboard():
    return render_template("dashboard.html", grafana_url=config.grafana_url)
